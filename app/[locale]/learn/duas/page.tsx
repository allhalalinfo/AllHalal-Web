...
export default async function DuasPage(props: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="container py-32 max-w-4xl mx-auto min-h-screen">
      ...
      <h1 className="text-4xl md:text-5xl font-bold text-text-primary mb-12">Duas & Athkar</h1>
      ...
      <div className="space-y-16">
        {duaCategories.map(category => {
          ...
          return (
            <div key={category.id}>
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-text-primary mb-2">{category.name}</h2>
              ...
              <div className="space-y-6">
                {categoryDuas.map(dua => (
                  <div key={dua.id} className="bg-bg-card border border-border p-8 rounded-3xl shadow-sm">
                    <div className="text-3xl md:text-4xl font-bold text-text-primary mb-6 leading-loose text-right" dir="rtl">
                      {dua.arabic}
                    </div>
